...
% FLEXURE_GUI M-file for Flexure_GUI.fig
...
% Last Modified by GUIDE v2.5 08-May-2009 15:36:57
...
handles.controllerinfo.maxharmonic = 2;
...
%flexureinfo
handles.flexureinfo.Gnfreqs_flexure = [handles.controllerinfo.Gnfreqs(1) handles.controllerinfo.Gnfreqs(end)*handles.controllerinfo.maxharmonic];
...
set(handles.Gnfreqs_flexure,'string',num2str(handles.plateinfo.basefreq))
...
set(handles.time_per_update,'string',num2str(handles.daqinfo.cycles_per_update*handles.plateinfo.T))
...
set(handles.errortol,'string',num2str(handles.globalinfo.errortol))
...
if get(hObject,'value')
    ...
    %CHECK WHETHER Gn HAS ALL FREQS REQUIRED FOR Gn_FLEXURE.  IF NOT,
    %GENERATE Gn
    
    handles = GenerateGn_flexure(handles);
else
...
function Gnfreqs_flexure_Callback(hObject, eventdata, handles)
% hObject    handle to Gnfreqs_flexure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of Gnfreqs_flexure as text
%        str2double(get(hObject,'String')) returns contents of Gnfreqs_flexure as a double

handles.flexureinfo.Gnfreqs_flexure = eval(get(hObject,'String'));

guidata(hObject, handles)


% --- Executes during object creation, after setting all properties.
function Gnfreqs_flexure_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Gnfreqs_flexure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
if ~isempty(answer)
    ...
    %update plateinfo
    %check to see if Gnfreqs_flexure has to change due to new Gn
    basefreq = handles.plateinfo.basefreq;
