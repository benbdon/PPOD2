function handles = GenerateGn_flexure(handles)
...
errortol = handles.globalinfo.errortol;
...
for i = 1:numel(platesignals)
    ...
    %get d and u signals
    d = 0*handles.signalinfo.d;
    d(:,i) = handles.signalinfo.d(:,i);
    ...
    set(handles.controllerupdates,'string',num2str(controllerupdates))
        ...
        while norm(e_fft) > errortol && get(handles.run,'value')
