...
%globalinfo
handles.globalinfo.mode = 'PddControl';%can be 'Pddcontrol','diddControl','uControl',freqResponse',freqResponseFlexure'
handles.globalinfo.uSignals = {'u1','u2','u3','u4','u5','u6'};%control signals (pre-amplified voltages)
...
handles.signalinfo.uCyc = zeros(SPC, NCS);%one cycle of control data (columns correspond to controlSignals)
...
%calibrationinfo
[T_P2pmi, T_di2ami] = frameTransformations(handles);%conversion between local acceleration readings to plate accelerations??
...
switch signals
    ...
    case 'actuatorAccelerations'
        [handles.signalinfo.dddDesChar, handles.signalinfo.diddDesCyc] = desCycUpdater(handles);%udpate desired acceleration signal
    case 'actuatorVoltages'
...
if isempty(answer)
    ...
    for in = 1:length(freqsNew)
        for i = 1:NCS
            ...
            handles.signalinfo.PddDesCyc = 0*handles.signalinfo.uCyc;
            ...
            for out = 1:NOHPF
                ...
                G_u2accNew(:,i,in,out) = PddCyc_fft(fftIndOut,:)/uCyc_fft(fftIndIn,i);
                G_u2accLocalNew(:,i,in,out) = pmiddCyc_fft(fftIndOut,:)/uCyc_fft(fftIndIn,i);
                G_u2dddNew(:,i,in,out) = diddCyc_fft(fftIndOut,:)/uCyc_fft(fftIndIn,i);
            end
        end
    end
...
[T_P2pmi, T_ai2ami, T_pi2pmi] = frameTransformations(handles);%conversion between local acceleration readings to plate accelerations??
handles.calibrationinfo.T_P2pmi = T_P2pmi;
handles.calibrationinfo.T_ai2ami = T_ai2ami;
handles.calibrationinfo.T_pi2pmi = T_pi2pmi;

handles.signalinfo.PddCyc = zeros(handles.signalinfo.samplesPerCycle, numel(handles.globalinfo.PddSignals));%one cycle of plate acceleration data (columns correspond to PddSignals)
handles.signalinfo.PddDesCyc = zeros(handles.signalinfo.samplesPerCycle, numel(handles.globalinfo.PddSignals));%one cycle of desired plate acceleration data (columns correspond to PddSignals)
handles.signalinfo.pmiddCyc = zeros(handles.signalinfo.samplesPerCycle, numel(handles.globalinfo.pmiddSignals));%one cycle of local plate acceleration data (columns correspond to pmiddSignals)
handles.signalinfo.diddCyc = zeros(handles.signalinfo.samplesPerCycle, numel(handles.globalinfo.diddSignals));%one cycle of actuator acceleration data (columns correspond to diddSignals)
handles.signalinfo.diddDesCyc = zeros(handles.signalinfo.samplesPerCycle, numel(handles.globalinfo.diddSignals));%one cycle of actuator acceleration data (columns correspond to diddSignals)
...
switch get(hObject,'tag')
    ...
    case 'actuatorAccelerations'
        [handles.signalinfo.dddDesChar, handles.signalinfo.diddDesCyc] = desCycUpdater(handles);
...
handles.signalinfo.PddDesCyc = zeros(handles.signalinfo.samplesPerCycle, numel(handles.globalinfo.PddSignals));%one cycle of desired plate acceleration data (columns correspond to PddSignals)
handles.signalinfo.pmiddCyc = zeros(handles.signalinfo.samplesPerCycle, numel(handles.globalinfo.pmiddSignals));%one cycle of local plate acceleration data (columns correspond to pmiddSignals)
handles.signalinfo.diddCyc = zeros(handles.signalinfo.samplesPerCycle, numel(handles.globalinfo.diddSignals));%one cycle of desired plate acceleration data (columns correspond to PddSignals)
handles.signalinfo.diddDesCyc = zeros(handles.signalinfo.samplesPerCycle, numel(handles.globalinfo.diddSignals));%one cycle of desired plate acceleration data (columns correspond to PddSignals)
