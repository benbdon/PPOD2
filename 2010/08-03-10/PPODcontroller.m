...
if strcmp(get(get(handles.desiredSignalSelector,'selectedobject'),'tag'),'actuatorVoltages')
    k = 0;
else
...
while currentUpdate < maxUpdate && get(handles.run,'value')
    ...
    while get(handles.daqinfo.ao,'samplesavailable') < 1*samplesPerCycle*(NCC+NPC)
        ...
        if strcmp(get(handles.daqinfo.ao,'Running'), 'Off')
            ...
            warndlg('Controller needs more processing time.  Increase processing cycles')
    ...
    if strcmp(get(get(handles.desiredSignalSelector,'selectedobject'),'tag'),'plateAccelerations')
        for i = 1:numel(plateAccSignals)
            accSignal = plateAccSignals{i};
            if ~strcmp(handles.signalinfo.desChar{i}, eval(['get(handles.desSignalChar',num2str(i),',''string'')']))
