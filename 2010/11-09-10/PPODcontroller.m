...
if get(handles.addFlexureFreqResponseData,'value')
    runmode = 'addFlexureFreqResponseData';
end
...
G_u2acc = zeros(numel(plateAccSignals),numel(controlSignals),max(controlledHarmonics),max(controlledHarmonics));
G_u2ddd = zeros(numel(actuatorAccSignals),numel(controlSignals),max(controlledHarmonics),max(controlledHarmonics));
if ~get(handles.addFreqResponseData,'value')
        ...
        if ~isempty(indIn)
            ...
            for out = in:in:harmonicsPerFreq*in
                if ismember(out,controlledHarmonics)
                    G_u2ddd(:,:,in,out) = G_u2ddd_all(1:NCS,1:NAAS,indIn,indOut);
                    G_u2acc(:,:,in,out) = G_u2acc_all(1:NCS,1:NPAS,indIn,indOut);
                    indOut = indOut + 1;
                end
            end
        else
...
while currentUpdate < maxUpdate
    ...
    switch handles.globalinfo.mode
        case 'standard'
            aLocalNCC = 1/V_per_ms2*aidata(:,1:NPALS);
            dddNCC = 1/V_per_ms2*aidata(:,NPALS+1:NPALS+NAAS);
