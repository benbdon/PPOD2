...
% Last Modified by GUIDE v2.5 28-Jul-2010 14:41:40
...
if T ~= handles.plateinfo.T
    ...
    if isempty(F(f1 == F))
        ...
        N_t = length(tCyc);
        handles.signalinfo.aCyc = zeros(N_t,numel(plateAccSignals));
        handles.signalinfo.aLocalCyc = zeros(N_t, numel(plateAccLocalSignals));
        handles.signalinfo.dddCyc = zeros(N_t, numel(actuatorAccSignals));
        handles.signalinfo.uCyc = zeros(N_t, numel(controlSignals));
        handles.signalinfo.adesCyc = zeros(N_t, numel(plateAccSignals));
