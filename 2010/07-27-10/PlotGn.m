...
actuatorsignals = mainGUIhandles.globalinfo.actuatorsignals;
...
set(handles.controlledfreqs,'string',[mat2str(Gnfreqs), ' Hz'])
...
for i = 1:6
    switch i
        ...
        case 6 %e.g., u --> y_sp1
            for j = 1:numel(actuatorsignals)
                plotoptions{i,j} = ['u_2_',actuatorsignals{j}];
            end
    end
end
...
if exist('panel')
    switch panel
        ...
        case 5 %panel 5: %e.g., u1 --> y_sp (1 input --> 6 outputs)
            if ~ismempty(actuatorsignals)
                for i = 1:numel(actuatorsignals)
                    %gain plots
                    eval(['axes(handles.axes',num2str(2*i-1),')'])
                    ...
                    title([controlsignals{ind},' --> ',actuatorsignals{i}])
                    if i == 1
                        ylabel(ylabeltext{1})
                    end
        ...
        case 6  %panel 6: %e.g., u --> y_sp1
            if ~ismempty(actuatorsignals)
                ...
                for i = 1:numel(controlsignals)
                    ...
                    title([controlsignals{i},' --> ',actuatorsignals{ind}])
                    if i == 1
                        ylabel(ylabeltext{1})
                    end
