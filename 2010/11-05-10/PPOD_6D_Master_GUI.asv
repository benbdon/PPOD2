...
handles.daqinfo.mode = [];%can be 'standard' or 'force'
...
%flexureinfo
flexureFreqResponseData = load([cd,'\freqResponseData\flexureFreqResponseData']);%load data for the four variables below
handles.flexureinfo.F_all = flexureFreqResponseData.F_all; %set of frequencies for which responses have been measured
handles.flexureinfo.G_acc2f_all = flexureFreqResponseData.G_acc2f_all;%set of discrete transfer functions from u to xyzrpy plate acceleration
handles.flexureinfo.Alin = flexureFreqResponseData.Alin;
handles.flexureinfo.Aang = flexureFreqResponseData.Aang;
...
if get(hObject,'value')
    ...
    answer = inputdlg({'New frequencies to add (Hz):';'Linear amplitudes (m/s^2)';'Angular amplitudes (rad/s^2)'},'Flexure Frequency Response Data Collection',[1],{'20';mat2str(Alin);mat2str(Aang)});
    ...
    if isempty(answer)
        ...
        %reset radio buttons for duration of transfer function generation
        set(handles.plateAccelerations,'value',1)
        set(handles.userSpecified,'enable','on','value',1)
        ...
        NOHPF = size(handles.flexureinfo.G_acc2f_all,4); %number of harmonics recorded per frequency
        ...
        for in = 1:length(freqsNew)
            for i = 1:NCS
                ...
                for out = 1:NOHPF
                    fftIndIn = 2; %always want first harmonic of input signal
                    fftIndOut = out+1; %cycle through harmonics of output signal
                    G_acc2fNew(:,i,in,out) = fCyc_fft(fftIndOut,:)/aCyc_fft(fftIndIn,i);
                end
            end
        end
        ...
        for in = 1:length(F_all)
            if ismember(in,indsNew)
                ...
                handles.flexureinfo.G_acc2f_all(:,:,in,:) = G_acc2fNew(:,:,ind,:);
            else
                ind = find(freqsOld==F_all(in));
                handles.flexureinfo.F_all(in) = freqsOld(ind);
                handles.flexureinfo.G_acc2f_all(:,:,in,:) = G_acc2fOld(:,:,ind,:);
            end
        end
        
        G_acc2f_all = handles.flexureinfo.G_acc2f_all;
        save([cd,'\freqResponseData\flexureFreqResponseData'],'F_all','G_acc2f_all','Alin','Aang')
