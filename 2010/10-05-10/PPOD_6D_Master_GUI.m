...
% Last Modified by GUIDE v2.5 05-Oct-2010 14:43:54
...
handles.calibrationinfo.FsensorAngleOffset = 70; %offset for force sensor angle in degrees 

%camerainfo
handles.camerainfo.fps = f1;

%flexureinfo
handles.flexureinfo.F_all = f1;
handles.flexureinfo.Alin = [5 5 5];
handles.flexureinfo.Aang = [50 50 50];
handles.flexureinfo.G_u2acc_all = freqResponseData.G_u2acc_all;
handles.flexureinfo.G_acc2f_all = [];
handles.flexureinfo.f_all = NaN*zeros(numel(handles.globalinfo.forceSignals),handles.signalinfo.samplesPerCycle,numel(handles.flexureinfo.flexureFreqs),numel(handles.globalinfo.plateAccSignals)); %4-d matrix with all force data.  dim 1 (columns) = Fx through Mz.  dim 2 (rows) = fft of Fx through Mz for logged number of cycles.  dim-3 = frequencies.  dim-4 = plate inputs
...
set(handles.flexureFreqs,'string',mat2str([f1]))
set(handles.Alin,'string',mat2str(handles.flexureinfo.Alin))
set(handles.Aang,'string',mat2str(handles.flexureinfo.Aang))
...
Aang = handles.flexureinfo.Aang;
freqsOld = handles.flexureinfo.F_all;
answer = inputdlg({'New frequencies to add (Hz):';'Linear amplitudes (m/s^2)';'Angular amplitudes (rad/s^2)'},'Flexure Frequency Response Data Collection',[1],{'';mat2str(Alin);mat2str(Aang)});
...
if isempty(answer)
    ...
    handles.flexureinfo.Aang = eval(answer{3});
    
    F_all = sort(unique([freqsOld,freqsNew]));
    indsNew = find(ismember(F_all,freqsNew));
    
    G_acc2fOld = handles.flexureinfo.G_acc2f_all;
    ...
    %reset radio buttons for duration of transfer function generation
    set(handles.actuatorAccelerations,'value',1)
    set(handles.userSpecified,'enable','on','value',1)
    ...
    NOHPF = size(handles.controllerinfo.G_acc2f_all,4); %number of harmonics recorded per frequency
    G_acc2fNew = zeros(NPAS,NFS,length(freqsNew), NOHPF);
    ...
    for in = 1:length(freqsNew)
        for i = 1:NCS
            ...
            Amps = [Alin Aang];
            handles.signalinfo.adesCyc = Amps(i)*sin(2*pi*f*tCyc);
            
            adesChari = [num2str(Amps(i)),'sin(2*pi*',char(sym(f)),'*t)'];
            eval(['set(handles.desSignalChar',num2str(i),',''string'',adesChari)'])
...
% --- Executes on button press in resetDAQflexure.
function resetDAQflexure_Callback(hObject, eventdata, handles)
% hObject    handle to resetDAQflexure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.daqinfo = Initializedaqinfo(handles);
set(handles.resetDAQflexure,'value',0)
