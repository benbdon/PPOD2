...
% Last Modified by GUIDE v2.5 12-Nov-2010 14:12:11
...
handles.globalinfo.mode = 'PddControl';%can be 'Pddcontrol','diddControl','uiControl','uiFreqResponse','diddFreqResponse'
...
handles.controllerinfo.F_ui = f1_ui*handles.controllerinfo.controlledHarmonics;%set of frequencies actively controlled
handles.controllerinfo.uMax = 6;%maximum allowed pre-amplified control voltage (saturation limit)
handles.controllerinfo.k = 0.25;%controller gain
handles.controllerinfo.GuiInputMag = .5;%input voltage used to generate transfer functions
handles.controllerinfo.GdiddInputMag = .5;%input actuator acceleration used to generate transfer functions
...
[NCS NPMIS NPS NAMIS NDIS NRFS NFS] = signalCounter(handles); %outputs number of signals
SPC = handles.signalinfo.samplesPerCycle;

handles.signalinfo.PddCyc = zeros(SPC, NPS);%one cycle of plate acceleration data (columns correspond to PddSignals)
handles.signalinfo.PddDesCyc = zeros(SPC, NPS);%one cycle of desired plate acceleration data (columns correspond to PddSignals)
handles.signalinfo.pmiddCyc = zeros(SPC, NPMIS);%one cycle of local plate acceleration data (columns correspond to pmiddSignals)
handles.signalinfo.amiddCyc = zeros(SPC, NAMIS);%one cycle of local actuator acceleration data (columns correspond to amiddSignals)
handles.signalinfo.diddCyc = zeros(SPC, NDIS);%one cycle of actuator acceleration data (columns correspond to diddSignals)
handles.signalinfo.diddDesCyc = zeros(SPC, NDIS);%one cycle of actuator acceleration data (columns correspond to diddSignals)
handles.signalinfo.uiCyc = zeros(SPC, NCS);%one cycle of control data (columns correspond to controlSignals)
...
set(handles.F_ui_all,'string',mat2str(handles.controllerinfo.F_ui_all))
...
if samplingFreq ~= handles.signalinfo.samplingFreq
    ...
    handles.signalinfo.uiCyc = zeros(length(tCyc), numel(controlSignals));
...
if T ~= handles.signalinfo.T
    ...
    if isempty(F_ui_all(f1_ui == F_ui_all))
        ...
        F = handles.controllerinfo.controlledHarmonics*f1_ui;
        discardFreqs = F(~ismember(F,F_ui_all));
        discardHarmonics = discardFreqs/f1_ui;
        F = F(ismember(F,F_ui_all));
        controlledHarmonics = F/f1_ui;
        ...
        set(handles.F_ui,'string',[mat2str(handles.controllerinfo.F),' Hz'])
        ...
        handles.signalinfo.samplesPerCycle = length(tCyc);
        handles.signalinfo.PddCyc = zeros(handles.signalinfo.samplesPerCycle,numel(PddSignals));
        handles.signalinfo.pmiddCyc = zeros(handles.signalinfo.samplesPerCycle, numel(pmiddSignals));
        handles.signalinfo.diddCyc = zeros(handles.signalinfo.samplesPerCycle, numel(diddSignals));
        handles.signalinfo.uiCyc = zeros(handles.signalinfo.samplesPerCycle, numel(controlSignals));
        handles.signalinfo.PddDesCyc = zeros(handles.signalinfo.samplesPerCycle, numel(PddSignals));
...
%answer = inputdlg({'New frequencies to add:';'Input voltage:'},'Frequency Response Data Collection',[1],{mat2str(F_ui);num2str(GInputMag)});
...
if isempty(answer)
    ...
    G_u2accOld = handles.controllerinfo.G_ui2Pdd_all;
    G_u2accLocalOld = handles.controllerinfo.G_ui2pmidd_all;
    G_u2dddOld = handles.controllerinfo.G_ui2didd_all;
    ...
    NPALS = length(handles.globalinfo.pmiddSignals);
    ...
    for in = 1:length(freqsNew)
        for i = 1:NCS
            ...
            handles.signalinfo.samplesPerCycle = length(tCyc);
            handles.signalinfo.uiCyc = zeros(handles.signalinfo.samplesPerCycle,NCS);
            handles.signalinfo.uiCyc(:,i) = GInputMag*sin(2*pi*f*tCyc);
            handles.signalinfo.PddDesCyc = 0*handles.signalinfo.uiCyc;
            handles.signalinfo.diddCyc = zeros(handles.signalinfo.samplesPerCycle,NAAS);
            handles.signalinfo.diddDesCyc = 0*handles.signalinfo.diddCyc;
            
            uiDesChari = [num2str(GInputMag),'sin(2*pi*',char(sym(f)),'*t)'];
            ...
            for out = 1:NOHPF
                ...
                G_u2accNew(:,i,in,out) = PddCyc_fft(fftIndOut,:)/uiCyc_fft(fftIndIn,i);
                G_u2accLocalNew(:,i,in,out) = pmiddCyc_fft(fftIndOut,:)/uiCyc_fft(fftIndIn,i);
                G_u2dddNew(:,i,in,out) = diddCyc_fft(fftIndOut,:)/uiCyc_fft(fftIndIn,i);
            end
        end
    end
    ...
    for in = 1:length(F_ui_all)
        if ismember(in,indsNew)
            ...
            handles.controllerinfo.G_ui2Pdd_all(:,:,in,:) = G_u2accNew(:,:,ind,:);
            handles.controllerinfo.G_ui2pmidd_all(:,:,in,:) = G_u2accLocalNew(:,:,ind,:);
            handles.controllerinfo.G_ui2didd_all(:,:,i,:) = G_u2dddNew(:,:,ind,:);
        else
            ind = find(freqsOld==F_ui_all(in));
            handles.controllerinfo.F_ui_all(in) = freqsOld(ind);
            handles.controllerinfo.G_ui2Pdd_all(:,:,in,:) = G_u2accOld(:,:,ind,:);
            handles.controllerinfo.G_ui2pmidd_all(:,:,in,:) = G_u2accLocalOld(:,:,ind,:);
            handles.controllerinfo.G_ui2didd_all(:,:,in,:) = G_u2dddOld(:,:,ind,:);
        end
    end
    ...
    switch handles.globalinfo.mode
        case 'standard'
            save([cd,'\freqResponseData\freqResponseData'],'F_ui_all','G_ui2Pdd_all','G_ui2pmidd_all','G_ui2didd_all','GInputMag')
        case 'force'
            save([cd,'\freqResponseData\freqResponseDataForce'],'F_ui_all','G_ui2Pdd_all','G_ui2pmidd_all','G_ui2didd_all','GInputMag','-append')
    end
...
Fdelete = eval(answer);
if ~isempty(Fdelete)
    handles.controllerinfo.F_ui_all(Fdelete) = [];
    handles.controllerinfo.G_ui2Pdd_all(:,:,Fdelete,:) = [];
    handles.controllerinfo.G_ui2pmidd_all(:,:,Fdelete,:) = [];
    handles.controllerinfo.G_ui2didd_all(:,:,Fdelete,:) = [];
end
