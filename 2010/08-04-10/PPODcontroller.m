...
while currentUpdate < maxUpdate && (get(handles.run,'value') || get(handles.addFreqResponseData,'value'))
    ...
    for i = 1:numel(plateAccSignals)
        desChariOld = handles.signalinfo.desChar{i};
        desChariNew = eval(['get(handles.desSignalChar',num2str(i),',''string'')']);
        if ~strcmp(desChariOld, desChariNew)
            handles.signalinfo.desChar{i} = desChariNew;
