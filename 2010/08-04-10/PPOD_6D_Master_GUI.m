...
if samplingFreq ~= handles.signalinfo.samplingFreq
    ...
    %update daqinfo
    handles.controllerinfo.samplingFreq = samplingFreq;
    ...
    handles.signalinfo.y = zeros(length(tCyc),numel(plateAccSignals));
    handles.signalinfo.y_raw = zeros(length(tCyc), numel(plateAccLocalSignals));
    handles.signalinfo.y_sp = zeros(length(tCyc), numel(actuatorAccSignals));
    handles.signalinfo.u = zeros(length(tCyc), numel(controlSignals));
    handles.signalinfo.d = zeros(length(tCyc), numel(plateAccSignals));
    ...
    set(handles.samplesPerCycle,'string',num2str(handles.signalinfo.samples_per_cycle))
...
F = handles.controllerinfo.F;
answer = inputdlg({'Enter new frequencies for data collection:';'Enter input voltage:'},'Input Frequencies',[1],{mat2str(F);num2str(GInputMag)});
...
if isempty(answer)
    ...
    G_u2accNew = zeros(length(handles.globalinfo.plateAccSignals),length(handles.globalinfo.controlSignals),length(freqsNew));
    G_u2accLocalNew = zeros(length(handles.globalinfo.plateAccLocalSignals),length(handles.globalinfo.controlSignals),length(freqsNew));
    G_u2dddNew = zeros(length(handles.globalinfo.actuatorAccSignals),length(handles.globalinfo.controlSignals),length(freqsNew));
    ...
    for j = 1:length(freqsNew)
        for i = 1:numel(handles.globalinfo.controlSignals)
            ...
            utext = [num2str(GInputMag),'sin(2*pi*',char(sym(f)),'*t)'];
            eval(['set(handles.desSignalChar',num2str(i),',''string'',utext)'])
            ...
            ind = 2;
            G_u2accNew(:,i,j) = aCyc_fft(ind,:)/uCyc_fft(ind,i);
            G_u2accLocalNew(:,i,j) = aLocalCyc_fft(ind,:)/uCyc_fft(ind,i);
            G_u2dddNew(:,i,j) = dddCyc_fft(ind,:)/uCyc_fft(ind,i);
        end
    end
...
switch get(hObject,'tag')
    ...
    case 'actuatorVoltages'
        ...
        handles.signalinfo.adesCyc = 0*handles.signalinfo.adesCyc;
