...
switch tag
    case 'guess'
        for i = 1:numel(controlledfreqs)
            ...
            u_fft(fftind,:) = (squeeze(Gn(:,:,Gnind))\d_fft(fftind,:).').';
...
samples_per_millisec = round(basefreq*samples_per_cycle*1e-3);
delay = 0;
if samples_per_millisec < samples_per_cycle
    for i = 1:N
        delay = mod(delay,samples_per_cycle);
        startind = delay + 1 + samples_per_cycle*(i-1);
        endind = startind + samples_per_millisec;
        delta = samples_per_cycle - (endind-i*samples_per_cycle); %check to see if pulse goes into the next cycle
        if delta > 0 %this is good--pulse stays in 1 cycle
            cam_trig(startind:endind) = 0;
        else
            cam_trig(i*samples_per_cycle:i*samples_per_cycle-delta)=0 ;cam_trig(startind:samples_per_cycle*i) = 0;
        end
        delay = delay + 1;
    end
else
...
while controllerupdates < maxupdates && get(handles.run,'value')
    ...
    %pl2x, pl2y)
    V_per_ms2 = .01; %volts per m/s^2 (NEED TO CALIBRATE)
    ...
    for i = 1:numel(controlledfreqs)  
        ...
        %get indices in fft signals corresponding to current
        %frequency
        fftind = Nb*i + 1;
