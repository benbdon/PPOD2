...
% Last Modified by GUIDE v2.5 10-Aug-2010 08:34:14
...
answer = inputdlg({'New frequencies to add:';'Harmonics collected per frequency:';'Input voltage:'},'Frequency Response Data Collection',[1],{mat2str(F);'5';num2str(GInputMag)});
...
if isempty(answer)
    ...
    if handles.controllerinfo.harmonicsPerFreq > 5
        warndlg(['Maximum harmonics per input frequency is 5.  Value will be set to 5.'])
    ...
    G_u2accNew = zeros(NPAS,NCS,length(freqsNew), handles.controllerinfo.harmonicsPerFreq);
    G_u2accLocalNew = zeros(NPALS,NCS,length(freqsNew),handles.controllerinfo.harmonicsPerFreq);
    G_u2dddNew = zeros(NAAS,NCS,length(freqsNew),handles.controllerinfo.harmonicsPerFreq);
    ...
    for in = 1:length(freqsNew)
        for i = 1:NCS
            ...
            NCC = 1; %default is 1 right now, code should be modified so that it can change
            for out = 1:handles.controllerinfo.harmonicsPerFreq
                fftIndIn = NCC + 1; %always want first harmonic of input signal
                fftIndOut = NCC*out+1; %cycle through harmonics of output signal
