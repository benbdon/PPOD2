...
handles.daqinfo.num_processing_cycles = 25;
...
handles.daqinfo.ao_channel_names = ['clock', handles.globalinfo.controlsignals, 'cam_trig'];
handles.daqinfo.ai_channel_names = handles.globalinfo.rawplatesignals;
num_outputs = numel(handles.daqinfo.ao_channel_names);
num_inputs = numel(handles.daqinfo.ai_channel_names);
...
if ismember('nidaq',daqinfo.InstalledAdaptors)
    ...
    handles.daqinfo.ai = analoginput('nidaq','Dev3');
    ...
    addchannel(handles.daqinfo.ao, 0:num_outputs-1,1:num_outputs, handles.daqinfo.ao_channel_names);
    addchannel(handles.daqinfo.ai, 0:num_inputs-1,1:num_inputs, handles.daqinfo.ai_channel_names);
