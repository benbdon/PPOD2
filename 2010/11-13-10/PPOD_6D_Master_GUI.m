...
% Last Modified by GUIDE v2.5 13-Nov-2010 13:06:02
...
handles.controllerinfo.uiHarmonics = 1:5;%set of harmonics actively controlled
...
set(handles.plottedHarmonics,'string',['[0:',num2str(max(10,max(handles.controllerinfo.uiHarmonics)*5)),']'],'enable','off')
...
set(handles.harmonicsCollectedPerFreq,'string',num2str(handles.controllerinfo.harmonicsCollectedPerFreq))
...
if T ~= handles.signalinfo.T
    ...
    if isempty(F_ui_all(f1_ui == F_ui_all))
        ...
        F = handles.controllerinfo.uiHarmonics*f1_ui;
        ...
        handles.signalinfo.uiCyc = zeros(handles.signalinfo.samplesPerCycle, numel(uiSignals));
...
%answer = inputdlg({'New frequencies to add:';'Input voltage:'},'Frequency Response Data Collection',[1],{mat2str(F_ui);num2str(GuiInputMag)});
answer = inputdlg({'New frequencies to add:';'Input voltage:'},'Frequency Response Data Collection',[1],{'20';num2str(GuiInputMag)});
...
if isempty(answer)
    ...
    set(handles.k,'enable','off')
    set(handles.harmonicsCollectedPerFreq,'enable','off')
    ...
    for in = 1:length(F_ui_all)
        if ismember(in,indsNew)
            ...
            handles.controllerinfo.G_ui2didd_all(:,:,i,:) = G_ui2diddNew(:,:,ind,:);
        else
    ...
    set(handles.k,'enable','on')
    set(handles.harmonicsCollectedPerFreq,'enable','on')
...
F_ui_all = handles.controllerinfo.F_ui_all;
answer = inputdlg({['Frequencies to delete: (',mat2str(F_ui_all),')']},'Delete Frequencies',1,{'[]'});
if ~isempty(answer)
    F_ui_delete = eval(answer{1});
    for i = 1:length(F_ui_delete)
        ind(i) = find(F_ui_all == F_ui_delete(i));
    end
    handles.controllerinfo.F_ui_all(ind) = [];
    handles.controllerinfo.G_ui2Pdd_all(:,:,ind,:) = [];
    handles.controllerinfo.G_ui2pmidd_all(:,:,ind,:) = [];
    handles.controllerinfo.G_ui2didd_all(:,:,ind,:) = [];
...
% Hint: get(hObject,'Value') returns toggle state of diddAddFreqs


% --- Executes on button press in diddDeleteFreqs.
function diddDeleteFreqs_Callback(hObject, eventdata, handles)
% hObject    handle to diddDeleteFreqs (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of diddDeleteFreqs
