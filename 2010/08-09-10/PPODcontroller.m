...
%1st dimension: row of G at fixed input and output harmonics
%2nd dimension: column of G at fixed input and output harmonics
%3rd dimension: input harmonic
%4th dimension: output harmonic
G_u2acc = zeros(6,6,max(controlledHarmonics),max(controlledHarmonics)*harmonicsPerFreq);

%4th dimension of G should only correspond to multiples of (1/T)
n = 1;
for N = 1:size(G_u2acc,4)
    f = n*f1;
    G_u2acc(:,:,:,n) = handles.controllerinfo.G_u2acc();
    n = n + 1;
end
%3rd dimension of G should only contain harmonicsPerFreq transfer
%functions, so set the rest to 0
for in = 1:size(G_u2acc,3)
    G_u2acc(:,:,in,harmonicsPerFreq*in+1:end) = zeros(6,6);
end
...
while currentUpdate < maxUpdate && (get(handles.run,'value') || get(handles.addFreqResponseData,'value'))
    ...
    for j = NCH
        fftInd1 = fftInds(j);
        fftInd2 = fftInds(end-j+1);
        FInd = FInds(j);
        n = controlledHarmonics(j);
        
        if n == min(controlledHarmonics)
            deltauNCC_fft = (G_u2acc(:,:,i,FInd)\eNCC_fft(fftInd1,:).').';
            uNCC_fft(fftInd1,:) = uNCC_fft(fftInd1,:) + k*deltauNCC_fft;
            uNCC_fft(fftInd2,:) = conj(uNCC_fft(fftInd1,:));
        else
            sumGdeltau = zeros(6,1);
            for i = max(1,n-harmonicsPerFreq+1):n-1
                sumGdeltau = sumGdeltau + G_u2acc(:,:,n,i)*deltauNCC_fft(fftInd1,:).';
            end
