...
% Last Modified by GUIDE v2.5 04-Aug-2010 11:08:28
...
handles.controllerinfo.harmonicsPerFreq = size(G_u2acc,3);
...
handles.controllerinfo.controlledFreqs = f1;%set of frequencies actively controlled
...
%DELETE ONCE SORTED OUT***********************
handles.controllerinfo.F = 30; %set of frequencies for which responses have been measured
handles.controllerinfo.G_u2acc = zeros(6,6,max(handles.controllerinfo.controlledHarmonics)*handles.controllerinfo.harmonicsPerFreq,numel(handles.controllerinof.F));%set of discrete transfer functions from u to xyzrpy plate acceleration
handles.controllerinfo.G_u2accLocal = zeros(12,6,1,1);%set of discrete transfer functions from u to plate accelerometers
handles.controllerinfo.G_u2ddd = zeros(6,6,1,1);%set of discrete transwer functions from u to actuator accelerometers
...
set(handles.plottedHarmonics,'string',['[0:',num2str(max(handles.controllerinfo.controlledHarmonics)*5),']'],'enable','off')
...
set(handles.F,'string',mat2str(handles.controllerinfo.F))
...
F = handles.controllerinfo.F;
answer = inputdlg({'New frequencies to add:';'Harmonics collected per frequency:';'Input voltage:'},'Frequency Response Data Collection',[1],{mat2str(F);'3';num2str(GInputMag)});
...
if isempty(answer)
    ...
    freqsOld = F;
    handles.controllerinfo.harmonicsPerFreq = eval(answer{2});
    handles.controllerinfo.GInputMag = eval(answer{3});
    
    F = sort(unique([freqsOld,freqsNew]));
    indsOld = find(ismember(F,freqsOld));
    
    G_u2accOld = handles.controllerinfo.G_u2acc;
    G_u2accLocalOld = handles.controllerinfo.G_u2accLocal;
    G_u2dddOld = handles.controllerinfo.G_u2ddd;
    
    %store current user-selections/value
    ...
    G_u2accNew = zeros(length(handles.globalinfo.plateAccSignals),length(handles.globalinfo.controlSignals),harmonicsPerFreq,length(freqsNew));
    G_u2accLocalNew = zeros(length(handles.globalinfo.plateAccLocalSignals),length(handles.globalinfo.controlSignals),harmonicsPerFreq,length(freqsNew));
    G_u2dddNew = zeros(length(handles.globalinfo.actuatorAccSignals),length(handles.globalinfo.controlSignals),harmonicsPerFreq,length(freqsNew));
    
    dt = 1/handles.signalinfo.samplingFreq;
    for j = 1:length(freqsNew)
        for i = 1:numel(handles.globalinfo.controlSignals)
            f = freqsNew(j);
            ...
            handles.signalinfo.samplesPerCycle = length(tCyc);
            handles.signalinfo.uCyc = zeros(handles.signalinfo.samplesPerCycle,length(handles.globalinfo.controlSignals));
            ...
            k = 1;
            for ind = 2:2+harmonicsPerFreq-1
                G_u2accNew(:,i,k,j) = aCyc_fft(ind,:)/uCyc_fft(ind,i);
                G_u2accLocalNew(:,i,k,j) = aLocalCyc_fft(ind,:)/uCyc_fft(ind,i);
                G_u2dddNew(:,i,k,j) = dddCyc_fft(ind,:)/uCyc_fft(ind,i);
                k = k + 1;
            end
        end
    end
    ...
    for i = 1:length(F)
        if ismember(i,indsOld)
            ind = find(freqsOld==F(i));
            handles.controllerinfo.F(i) = freqsOld(ind);
            handles.controllerinfo.G_u2acc(:,:,:,i) = G_u2accOld(:,:,:,ind);
            handles.controllerinfo.G_u2accLocal(:,:,:,i) = G_u2accLocalOld(:,:,:,ind);
            handles.controllerinfo.G_u2ddd(:,:,:,i) = G_u2dddOld(:,:,:,ind);
        else
            ind = find(freqsNew==F(i));
            handles.controllerinfo.F(i) = freqsNew(ind);
            handles.controllerinfo.G_u2acc(:,:,:,i) = G_u2accNew(:,:,:,ind);
            handles.controllerinfo.G_u2accLocal(:,:,:,i) = G_u2accLocalNew(:,:,:,ind);
            handles.controllerinfo.G_u2ddd(:,:,:,i) = G_u2dddNew(:,:,:,ind);
        end
    end
    
    
    G_u2acc = handles.controllerinfo.G_u2acc;
    G_u2accLocal = handles.controllerinfo.G_u2accLocal;
    G_u2ddd = handles.controllerinfo.G_u2ddd;
    save([cd,'\freqResponseData\freqResponseData'],'F','G_u2acc','G_u2accLocal','G_u2ddd','GInputMag')
    ...
    switch desSignal
        case 'plateAccelerations'
            ...
            set(handles.desSignalChar1,'string',handles.signalinfo.desChar{1})
            set(handles.desSignalChar2,'string',handles.signalinfo.desChar{2})
            set(handles.desSignalChar3,'string',handles.signalinfo.desChar{3})
            set(handles.desSignalChar4,'string',handles.signalinfo.desChar{4})
            set(handles.desSignalChar5,'string',handles.signalinfo.desChar{5})
            set(handles.desSignalChar6,'string',handles.signalinfo.desChar{6})
