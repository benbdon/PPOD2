...
if exist('panel')
    switch panel
        ...
        case 1 %panel 1: %e.g., u1 --> pmidd (1 input --> 6 outputs)
        ...
        case 2 %panel 2: %e.g., ui --> pddx (6 inputs --> 1 output)
            
            for i = 1:numel(uiSignals)
                %gain plots
                eval(['axes(handles.axes',num2str(2*i-1),')'])
                stem(freqs, squeeze(gain_ui2Pdd(ind,i,:,1)))
                set(gca,'xlim',[0 F_ui_all(end)+10])
                if ind <= 3
                    set(gca,'Ylim',[1.1*ymin 1.1*ymax])
                    if i == 1
                ...
                %phase plots
                eval(['axes(handles.axes',num2str(2*i),')'])
                stem(freqs, unwrap(squeeze(G_ui2Pdd_phase(ind,i,:,1))))
                set(gca,'xlim',[0 F_ui_all(end)+10])
                if strcmp(frequencyunits,'rad_per_s')
                    set(gca,'Xscale','log')
                end
        ...
        case 3 %panel 3: %e.g., u1 --> pmidd (1 input --> 12 outputs)
        ...
        case 4  %panel 4: %e.g., ui --> pm1ddx (6 inputs --> 1 output)
            
            for i = 1:numel(uiSignals)
                %gain plots
                eval(['axes(handles.axes',num2str(2*i-1),')'])
                stem(freqs, squeeze(gain_ui2pmidd(ind,i,:,1)))
                set(gca,'ylim',[1.1*ymin_raw 1.1*ymax_raw])
                set(gca,'xlim',[0 F_ui_all(end)+10])
                if strcmp(frequencyunits,'rad_per_s')
                    set(gca,'Xscale','log')
                ...
                %phase plots
                eval(['axes(handles.axes',num2str(2*i),')'])
                stem(freqs, unwrap(squeeze(G_ui2pmidd_phase(ind,i,:,1))))
                set(gca,'xlim',[0 F_ui_all(end)+10])
                if strcmp(frequencyunits,'rad_per_s')
                    set(gca,'Xscale','log')
                end
        ...
        case 5 %panel 5: %e.g., u1 --> didd (1 input --> 6 outputs)
        ...
        case 6  %panel 6: %e.g., ui --> d1dd
            ...
            for i = 1:numel(uiSignals)
                %gain plots
                eval(['axes(handles.axes',num2str(2*i-1),')'])
                stem(freqs, squeeze(gain_ui2didd(ind,i,:,1)))
                set(gca,'ylim',[1.1*ymin_sp 1.1*ymax_sp])
                set(gca,'xlim',[0 F_ui_all(end)+10])
                if strcmp(frequencyunits,'rad_per_s')
                    set(gca,'Xscale','log')
                ...
                %phase plots
                eval(['axes(handles.axes',num2str(2*i),')'])
                stem(freqs, unwrap(squeeze(G_ui2didd_phase(ind,i,:,1))))
                set(gca,'xlim',[0 F_ui_all(end)+10])
                if strcmp(frequencyunits,'rad_per_s')
                    set(gca,'Xscale','log')
                end
