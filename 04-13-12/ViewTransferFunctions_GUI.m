...
% Last Modified by GUIDE v2.5 04-Mar-2011 10:01:42
...
%update plotted freqs
set(handles.plottedFreqs,'string',mat2str(handles.controllerinfo.F_ui_all))
...
plottedFreqs = eval(get(handles.plottedFreqs,'string'));

switch inputSignals
    case 'inputSignals_ui'
        F_all = handles.controllerinfo.F_ui_all;
        [F_plot, F_ind] = intersect(F_all,plottedFreqs);
        switch outputSignals
            case 'outputSignals_amidd'
                G = handles.controllerinfo.G_ui2amidd_all(:,:,F_ind,:);
            case 'outputSignals_didd'
                G = handles.controllerinfo.G_ui2didd_all(:,:,F_ind,:);
            case 'outputSignals_pmidd'
                G = handles.controllerinfo.G_ui2pmidd_all(:,:,F_ind,:);
            case 'outputSignals_Pdd'
                G = handles.controllerinfo.G_ui2Pdd_all(:,:,F_ind,:);
            case 'outputSignals_f'
                G = handles.controllerinfo.G_ui2f_all(:,:,F_ind,:);
            otherwise
                error('selection does not match any case')
        end
    ...
    case 'inputSignals_didd'
        ...
        switch outputSignals
            case 'outputSignals_amidd'
                G = handles.controllerinfo.G_didd2amidd_all(:,:,F_ind,:);
            case 'outputSignals_didd'
                G = handles.controllerinfo.G_didd2amidd_all(:,1:NDIDDS,F_ind,:);
            case 'outputSignals_pmidd'
                G = handles.controllerinfo.G_didd2pmidd_all(:,:,F_ind,:);
            case 'outputSignals_Pdd'
                G = handles.controllerinfo.G_didd2Pdd_all(:,:,F_ind,:);
            case 'outputSignals_f'
                G = handles.controllerinfo.G_didd2f_all(:,:,F_ind,:);
            otherwise
                error('selection does not match any case')
        end
    case 'inputSignals_Pdd'
        ...
        switch outputSignals
            case 'outputSignals_f'
                G = handles.controllerinfo.G_Pdd2f_all(:,:,F_ind,:);
            otherwise
                error('selection does not match any case')
        end
    otherwise
...
magGmax = max(max(max(max(magG(outputInds,:,:,:)))));
magGmin = min(min(min(min(magG(outputInds,:,:,:)))));
if strcmp(outputSignals, 'outputSignals_Pdd') && numel(outputInds) == 6
    magGmax_all(1:3) = max(max(max(max(magG(outputInds(1:3),:,:,:)))));
    magGmax_all(4:6) = max(max(max(max(magG(outputInds(4:6),:,:,:)))));
    magGmin_all(1:3) = min(min(min(max(magG(outputInds(1:3),:,:,:)))));
    magGmin_all(4:6) = min(min(min(max(magG(outputInds(4:6),:,:,:)))));
else
    magGmax_all(1:max(numOutputs,numInputs)) = magGmax;
    magGmin_all(1:max(numOutputs,numInputs)) = magGmin;
end

magBorder_all = (magGmax_all-magGmin_all)/10;
magylim_all = [magGmin_all-magBorder_all; magGmax_all+magBorder_all]';
...
plottype = get(get(handles.plottypeSelector,'selectedObject'),'tag');
...
for i = 1:numOutputs
    for j = 1:numInputs
        switch outputSignals
            case 'outputSignals_pmidd'
                if numOutputs < numel(handles.globalinfo.pmiddSignals)
                    ...
                    phaseG_plot(:,1) = unwrap(squeeze(phaseG(2*outputInds(i)-1,inputInds(j),:,outputHarmonic)));
                    magG_plot(:,2) = squeeze(magG(2*outputInds(i),inputInds(j),:,outputHarmonic));
                    phaseG_plot(:,2) = unwrap(squeeze(phaseG(2*outputInds(i),inputInds(j),:,outputHarmonic)));
                    legText = {'x','y'};
                else
                    magG_plot = squeeze(magG(outputInds(i),inputInds(j),:,outputHarmonic));
                    phaseG_plot = unwrap(squeeze(phaseG(outputInds(i),inputInds(j),:,outputHarmonic)));
                end
                
            case 'outputSignals_amidd'
                [NUIS NPMIDDS NPDDS NAMIDDS NDIDDS] = signalCounter(handles);
                if numOutputs < numel(handles.globalinfo.amiddSignals)
                    magG_plot(:,1) = squeeze(magG(outputInds(i),inputInds(j),:,outputHarmonic));
                    phaseG_plot(:,1) = unwrap(squeeze(phaseG(outputInds(i),inputInds(j),:,outputHarmonic)));
                    magG_plot(:,2) = squeeze(magG(outputInds(i)+NDIDDS,inputInds(j),:,outputHarmonic));
                    phaseG_plot(:,2) = unwrap(squeeze(phaseG(outputInds(i)+NDIDDS,inputInds(j),:,outputHarmonic)));
                    legText = {'x','y'};
                else
                    magG_plot = squeeze(magG(outputInds(i),inputInds(j),:,outputHarmonic));
                    phaseG_plot = unwrap(squeeze(phaseG(outputInds(i),inputInds(j),:,outputHarmonic)));
        ...
        switch plottype
            case 'continuous'
                plot(F_plot,magG_plot)
            case 'discrete'
                stem(F_plot,magG_plot)
        end
       
        %%%set(gca,'ylim',magylim_all(i*j,:))
        switch freqUnits
            case 'freqUnit_rad'
                eval(['set(handles.axes',num2str(2*i*j-1),',''xscale'',''log'')'])
                eval(['set(handles.axes',num2str(2*i*j),',''xscale'',''log'')'])
            case 'freqUnit_Hz'
                eval(['set(handles.axes',num2str(2*i*j-1),',''xscale'',''linear'')'])
                eval(['set(handles.axes',num2str(2*i*j),',''xscale'',''linear'')'])
            otherwise
                error('selection does not match any case')
        end
        ...
        switch plottype
            case 'continuous'
                plot(F_plot,phaseG_plot)
            case 'discrete'
                stem(F_plot,phaseG_plot)
        end
        
        %set(gca,'ylim',[-1.5*pi 1.5*pi])
        switch freqUnits
            case 'freqUnit_rad'
                eval(['set(handles.axes',num2str(2*i*j-1),',''xscale'',''log'')'])
            case 'freqUnit_Hz'
...
set(handles.plottedFreqs,'string',mat2str(F_plot))

% --- Executes when selected object is changed in plottypeSelector.
function plottypeSelector_SelectionChangeFcn(hObject, eventdata, handles)
% hObject    handle to the selected object in plottypeSelector 
% eventdata  structure with the following fields (see UIBUTTONGROUP)
%	EventName: string 'SelectionChanged' (read only)
%	OldValue: handle of the previously selected object or empty if none was selected
%	NewValue: handle of the currently selected object
