...
while currentUpdate < maxUpdate
    ...
    if strcmp(handles.globalinfo.aiConfig,'force')
        ...
        %transform wrench from force sensor frame to frame centered at one end of
        %flexure
        fNCC = (T_F2W*fNCC')';
    ...
    if get(handles.squareSequence,'value')
        fields = {'TransX','TransNegY','TransNegX','TransY','Sink'};
        numupdates = [2,2,2,1,1];%num updates for each field
        ...
        for kk = 1:length(fields)
            if rem(currentUpdate,updatespercycle) == updateindices(kk)
                set(handles.savedSignalsListbox,'Value',find(strncmp(fields{kk},savedfieldnames,length(fields{kk}))))
    ...
    if get(handles.sinkSourceSequence,'value')
        ...
        numupdates = [4,3,5,3];%num updates for each field
        updateindices = [0 cumsum(numupdates(1:end-1))];
        updatespercycle = sum(numupdates);
        for kk = 1:length(fields)
            if rem(currentUpdate,updatespercycle) == updateindices(kk)
    ...
    if get(handles.samplerSequence,'value')
        fields = {'LineSinkX','Source','Saddle','TransNegY','SqueezeTransY','TransNegX','Circle'};
        numupdates = [10,3,10,5,12,4,10];%num updates for each field
        updateindices = [0 cumsum(numupdates(1:end-1))];
        updatespercycle = sum(numupdates);
        for kk = 1:length(fields)
            if rem(currentUpdate,updatespercycle) == updateindices(kk)
    ...
    switch handles.globalinfo.mode
        case 'PddControl'
            ...
            %deal with user selecting a saved signals from listbox
    ...
    switch handles.globalinfo.mode
        ...
        case 'PddAddFreqs'
            maxUpdate = eval(get(handles.maxUpdatePddAddFreqs,'string'));
    end
